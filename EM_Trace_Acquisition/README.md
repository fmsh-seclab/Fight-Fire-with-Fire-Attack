...
To identify the optimal location for EM signal capture, multiple positions on the SoC were tested using probes of various shapes. The `Langer EMV RF-R 3-2` probe positioned at the location indicated in Figure below captured distinctive signals. These signals were closely correlated with the signature verification and firmware decryption operations.
