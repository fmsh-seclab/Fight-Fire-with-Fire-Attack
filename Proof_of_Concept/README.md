...
- **`em_trace/`**: Contains the EM trace-set captured during the MB1 firmware decryption process using SBK.
 ```  
    ├── EM_Trace_SBK_Decryption.trs   
    |   ######### 8 EM trace captured during SBK decryption Phase ########
    ├── EM_Trace_SBK_Decryption_Single_Block_Extraction.trs         
    |   ######### SBK Trace Set after single block extraction ########
    ├── EM_Trace_SBK_Decryption_Block_Extraction_POI_15.trs        
        ######## Simplified SBK Trace Set (The 15th Sample Point of the single block Trace set)######## 
  ```
...
Key Recovery Results as follow:
  ...
  ```
## Validation Result
Confirm that the decrypted result is identical to the original unencrypted file. 

For instance, the MB1 firmware in `BSP version 32.4.2` was successfully decrypted, with its output perfectly matching the original unaltered payload. 

Left figure: `Original MB1 file` 
Right figure: `Decrypted file`
